...
<div class="row mb-3">
    <div class="col-12 mb-3">
        <form action="{{ route('ips.index') }}" method="GET">
            <div class="d-flex flex-wrap gap-2" style="gap: 10px;">
                <input type="text" name="search" class="form-control theme-input" placeholder="{{ __('messages.search_ip') }}" value="{{ request('search') }}" style="width: 250px;" >
                ...
                <select name="status" class="form-control select2 theme-input" style="width: 150px; background-color: rgba(0,0,0,0.2); color: #fff; border: 1px solid rgba(255,255,255,0.1);" >
                    ...
                    <option value="Used" style="color: #000;" {{ request('status') == 'Used' ? 'selected' : '' }}>{{ __('messages.used') }}</option>
                    <option value="Reserved" style="color: #000;" {{ request('status') == 'Reserved' ? 'selected' : '' }}>{{ __('messages.reserved') }}</option>
                </select>
                
                <button type="submit" class="btn btn-primary" ><i class="fas fa-search"></i></button>
                @if(request()->anyFilled(['search', 'status']))
                    <a href="{{ route('ips.index') }}" class="btn btn-outline-secondary" ><i class="fas fa-undo"></i> {{ __('messages.reset') }}</a>
                @endif
            </div>
        </form>
    </div>
    ...
    <div class="col-12 d-flex justify-content-end" style="gap: 10px;">
        <a href="{{ route('ips.export') }}" class="btn btn-sm btn-success"><i class="fas fa-file-excel"></i> {{ __('messages.export') }}</a>
        <a href="{{ route('ips.create') }}" class="btn btn-sm btn-primary" ><i class="fas fa-plus"></i> {{ __('messages.add_ip') }}</a>
    </div>
</div>
            ...
            <table class="table table-hover mb-0 theme-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="theme-text">
                            @if($ip->status == 'Available') <span class="badge badge-success" style="box-shadow: 0 0 8px rgba(40,167,69,0.5);">{{ __('messages.available') }}</span>
                            @elseif($ip->status == 'Used') <span class="badge badge-primary" style="box-shadow: 0 0 8px rgba(0,123,255,0.5);">{{ __('messages.used') }}</span>
                            @else <span class="badge badge-warning" style="box-shadow: 0 0 8px rgba(255,193,7,0.5);">{{ __('messages.reserved') }}</span>
                            @endif
                        </td>
                        <td class="theme-text">
                            <div class="d-flex justify-content-center" style="gap: 8px;">
                                <button type="button" class="btn action-btn btn-outline-success ping-btn" data-ping-url="{{ route('ips.ping', $ip) }}" title="Ping Device" style="border: 1px solid rgba(40, 167, 69, 0.3); background: rgba(40, 167, 69, 0.15); color: #28a745;"><i class="fas fa-play"></i></button>
                                <a href="{{ route('ips.edit', $ip) }}" class="btn action-btn btn-edit-tech"  title="{{ __('messages.edit') }}"><i class="fas fa-edit"></i></a>
