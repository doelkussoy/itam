        ...
        <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-md-6 form-group">
                    ...
                    <select name="status" class="form-control @error('status') is-invalid @enderror" required >
                        <option value="Available" style="color: #000;" {{ old('status') == 'Available' ? 'selected' : '' }}>{{ __('messages.available') }}</option>
                        <option value="Used" style="color: #000;" {{ old('status') == 'Used' ? 'selected' : '' }}>{{ __('messages.used') }}</option>
                        <option value="Reserved" style="color: #000;" {{ old('status') == 'Reserved' ? 'selected' : '' }}>{{ __('messages.reserved') }}</option>
                    </select>
            ...
            <div class="row">
                <div class="col-md-6 form-group">
                    ...
                    <input type="text" name="mac_address" class="form-control @error('mac_address') is-invalid @enderror" value="{{ old('mac_address') }}" placeholder="e.g. AA:BB:CC:DD:EE:FF" >
                ...
                <div class="col-md-6 form-group">
                    ...
                    <select name="vlan_id" class="form-control select2 @error('vlan_id') is-invalid @enderror" >
                        ...
                        @foreach($vlans as $vlan)
                            <option value="{{ $vlan->id }}" style="color: #000;" {{ old('vlan_id') == $vlan->id ? 'selected' : '' }}>
