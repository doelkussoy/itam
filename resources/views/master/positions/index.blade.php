...
@section('title', __('messages.manage') . ' ' . __('messages.position'))
        ...
        <form action="{{ route('positions.index') }}" method="GET">
            <div class="d-flex flex-wrap gap-2" style="gap: 10px;">
                ...
                @if(request('search') || request('department_id'))
                    <a href="{{ route('positions.index') }}" class="btn btn-outline-secondary" ><i class="fas fa-times"></i></a>
                @endif
            </div>
        </form>
    </div>
...
<div class="card theme-card">
    <div class="card-header border-0">
        <h3 class="card-title theme-text">{{ __('messages.list') }} {{ __('messages.position') }}</h3>
        <div class="card-tools d-flex" style="gap: 10px;">
            <a href="{{ route('positions.export') }}" class="btn btn-sm btn-success"><i class="fas fa-file-excel"></i> Export</a>
            ...
            <a href="{{ route('positions.create') }}" class="btn btn-sm btn-primary"><i class="fas fa-plus"></i> {{ __('messages.add_new') }}</a>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-striped table-hover m-0 theme-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="theme-text">{{ ($positions->currentPage() - 1) * $positions->perPage() + $loop->iteration }}</td>
                        <td class="theme-text">{{ $item->name }}</td>
                        <td class="theme-text">{{ $item->department->name ?? '-' }}</td>
                        <td class="theme-text">{{ $item->description }}</td>
                        <td class="theme-text">
                            <div class="d-flex" style="gap: 8px;">
                                <a href="{{ route('positions.edit', $item) }}" class="btn action-btn btn-edit-tech"  title="{{ __('messages.edit') }}"><i class="fas fa-edit"></i></a>
                    ...
                    @empty
                    <tr><td colspan="6" class="text-center text-muted">{{ __('messages.no_data') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="modal-content theme-card">
      <form action="{{ route('positions.import') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-header border-0">
          <h5 class="modal-title theme-text" id="importModalLabel">Import Positions</h5>
