...
<div class="card theme-card">
    <div class="card-header border-0">
        ...
        <div class="card-tools d-flex" style="gap: 10px;">
            <a href="{{ route('categories.export') }}" class="btn btn-sm btn-success"><i class="fas fa-file-excel"></i> Export</a>
        ...
        <div class="table-responsive">
            <table class="table table-striped table-hover m-0 theme-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="theme-text">{{ ($categories->currentPage() - 1) * $categories->perPage() + $loop->iteration }}</td>
                        <td class="theme-text">{{ $item->name }}</td>
                        <td class="theme-text">{{ $item->description }}</td>
                        <td class="theme-text">
                            <div class="d-flex" style="gap: 8px;">
                                <a href="{{ route('categories.edit', $item) }}" class="btn action-btn btn-edit-tech"  title="{{ __('messages.edit') }}"><i class="fas fa-edit"></i></a>
    ...
    <div class="modal-content theme-card">
      <form action="{{ route('categories.import') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-header border-0">
          <h5 class="modal-title theme-text" id="importModalLabel">Import Categories</h5>
