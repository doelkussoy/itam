...
@section('title', __('messages.edit') . ' ' . __('messages.vendor'))
    ...
    <form action="{{ route('vendors.update', $vendor) }}" method="POST">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <input type="text" name="contact_person" class="form-control @error('contact_person') is-invalid @enderror" value="{{ old('contact_person', $vendor->contact_person) }}" required>
                @error('contact_person') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label  class="theme-text">{{ __('messages.email') }}</label>
                <input type="text" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email', $vendor->email) }}" required>
