...
@section('title', __('messages.manage') . ' ' . __('messages.location'))
    ...
    <div class="col-12 mb-3">
        <form action="{{ route('locations.index') }}" method="GET">
            <div class="d-flex flex-wrap gap-2" style="gap: 10px;">
                ...
                @if(request('search'))
                    <a href="{{ route('locations.index') }}" class="btn btn-outline-secondary" ><i class="fas fa-times"></i></a>
                @endif
            </div>
        </form>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-striped table-hover m-0 theme-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="theme-text">{{ ($locations->currentPage() - 1) * $locations->perPage() + $loop->iteration }}</td>
    ...
    <div class="modal-content">
      <form action="{{ route('locations.import') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-header">
          <h5 class="modal-title" id="importModalLabel"><i class="fas fa-file-import text-info mr-2"></i> {{ __('messages.import_excel') }} {{ __('messages.location') }}</h5>
