...
@section('title', __('messages.edit_license') ?? 'Edit Software License')
    ...
    <form action="{{ route('software_licenses.update', $softwareLicense) }}" method="POST">
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <input type="text" name="name" class="form-control theme-input @error('name') is-invalid @enderror" value="{{ old('name', $softwareLicense->name) }}" required>
                @error('name') <span class="text-danger">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label class="theme-text">{{ __('messages.license_key') }}</label>
                <textarea name="license_key" class="form-control theme-input @error('license_key') is-invalid @enderror" rows="3">{{ old('license_key', $softwareLicense->license_key) }}</textarea>
                @error('license_key') <span class="text-danger">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label class="theme-text">{{ __('messages.expiry_date') }}</label>
                <input type="date" name="expiry_date" class="form-control theme-input @error('expiry_date') is-invalid @enderror" value="{{ old('expiry_date', $softwareLicense->expiry_date ? $softwareLicense->expiry_date->format('Y-m-d') : '') }}">
            ...
            <div class="form-group">
                ...
                <input type="number" name="total_seats" class="form-control theme-input @error('total_seats') is-invalid @enderror" value="{{ old('total_seats', $softwareLicense->total_seats) }}" min="1" required>
            ...
            <div class="form-group">
                ...
                <textarea name="notes" class="form-control theme-input @error('notes') is-invalid @enderror" rows="3">{{ old('notes', $softwareLicense->notes) }}</textarea>
